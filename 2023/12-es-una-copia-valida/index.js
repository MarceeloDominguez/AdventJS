...
// Y, como ves, la degradación de las letras no se produce en un orden específico, es aleatorio.

function checkIsValidCopy(original, copy) {
  const degrade = (char) => {
    const isUpperCase = /^[A-Z]+$/.test(char);
    char = [char, char.toLocaleLowerCase()][+isUpperCase];

    const characters = "#+:. ";
    const characterIndex = characters.indexOf(char);
    const isCharacter = characterIndex !== -1;
    char = [char + characters, characters.slice(characterIndex)][+isCharacter];
  ...
  for (const char of original) {
    const degradeChars = degrade(char);
    if (degradeChars !== " " && degradeChars !== copy[i++]) {
      return false;
    } else if (degradeChars === " " && copy[i] !== " ") {
      return false;
    }
  }
